...
bool SpirvEmitter::validateVKAttributes(const NamedDecl *decl) {
  ...
  //vk::shader_record_nv is supported only on cbuffer/ConstantBuffer(not on tbuffer/TextureBuffers
  //which are treated as HLSL
  if (const auto *srbAttr = decl->getAttr<VKShaderRecordNVAttr>()) {
    ...
    if (!isValidType) {
      emitError("vk::shader_record_nv can be applied only to cbuffer/ConstantBuffer", loc);
      success = false;
    }
    if (decl->hasAttr<VKBindingAttr>()) {
