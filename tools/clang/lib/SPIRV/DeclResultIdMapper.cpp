...
SpirvVariable *DeclResultIdMapper::createStructOrStructArrayVarOfExplicitLayout(
  ...
  const bool forShaderRecordNV = usageKind == ContextUsageKind::ShaderRecordBufferNV;
  ...
  const auto sc =
      forPC ? spv::StorageClass::PushConstant : forShaderRecordNV ? spv::StorageClass::ShaderRecordBufferNV :
...
SpirvVariable *DeclResultIdMapper::createShaderRecordBufferNV(const VarDecl *decl) {
  const auto *recordType = decl->getType()->getAs<RecordType>();
  assert(recordType);
  ...
  SpirvVariable *var = createStructOrStructArrayVarOfExplicitLayout(
      recordType->getDecl(), /*arraySize*/ 0, ContextUsageKind::ShaderRecordBufferNV,
      structName, decl->getName());
...
SpirvVariable *DeclResultIdMapper::createShaderRecordBufferNV(const HLSLBufferDecl *decl) {

  const std::string structName = "type.ShaderRecordBufferNV." + decl->getName().str();
  // The front-end does not allow arrays of cbuffer/tbuffer.
  SpirvVariable *bufferVar = createStructOrStructArrayVarOfExplicitLayout(
      decl, /*arraySize*/ 0, ContextUsageKind::ShaderRecordBufferNV, structName, decl->getName());
